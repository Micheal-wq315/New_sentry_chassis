...
//导航串口，发送比赛状态和血量状态 
    test_buff[0] = 0xAA;
    //非比赛中不接受导航
    if(game_state == 0 || game_state == 1 || game_state == 2 || game_state == 3 || game_state == 5){
        test_buff[1] = 0x00;
    }
    //开赛开始导航
    ...
    if(referee_data.GameRobotState.current_HP <= 120){
        test_buff[2] = 0x01;
    }
    else{
        test_buff[2] = 0x00;
    ...
    if(referee_data.GameRobotState.current_HP >= 300 && referee_data.GameRobotState.current_HP <= 400){
        test_buff[3] = 0x01;
    }
    else{
        test_buff[3] = 0x00;
...
//视觉串口，发送敌人颜色
    ...
    // for(int i = 0;i<=1;i++){
    //     Vision_buff[2] ^= Vision_buff[i];
    // }

    // Vision_buff[3] = 0x5A;

    HAL_UART_Transmit_DMA(&huart7, test_buff, sizeof(test_buff));
    //HAL_UART_Transmit_DMA(&huart10, Vision_buff, sizeof(Vision_buff));
}
