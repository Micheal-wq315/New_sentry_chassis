...
/*
txid为0x26，rxid为0x16，设置为速度模式是应因为海林哥写了手动位置闭环的逻辑，
在can发送帧中，达妙电机速度模式的标准can帧可见使用手册，所以我们只要发送前四位即可，
现在解禁了遥控器，但现在遥控器只用于调试。
改进方案：改电机为速度模式，电机的id正常，使能正常，can数据帧正常，但电机的模式设置有问题
*/
void DMMotorTask(void const *argument)
{
   ...
   //DM_Motor_Measure_s *measure = &motor->measure;
    ...
    {   
        ...
        // motor->motor_can_instace->tx_buff[0] = 0;
        // motor->motor_can_instace->tx_buff[1] = 0;
        // motor->motor_can_instace->tx_buff[2] = 0;
        // motor->motor_can_instace->tx_buff[3] = 40;
        ...
        CANTransmit(motor->motor_can_instace, 3);
